...
- **model**:
  ...
  > - 日常推荐 gpt-5-nano（约 0.35RMB/百万汉字）。
  > - 复杂问题和多模态处理用 gpt-5（约 8RMB/百万汉字）。
  >
- **temperature**:
...
3. **response_metadata**:
   ...
   > - **model_name**: 使用的模型名称，例如 `"gpt-5-nano-2024-07-18"`。
...
# 创建 ChatOpenAI 实例
llm = ChatOpenAI(model="gpt-5-nano", stream=True)
