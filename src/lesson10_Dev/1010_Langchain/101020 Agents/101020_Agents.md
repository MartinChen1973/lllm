...
> ℹ️ **How `create_agent` works** > [`create_agent`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent) builds a **graph**-based agent runtime using [LangGraph](/oss/python/langgraph/overview).
...
> Learn more in the [Graph API](/oss/python/langgraph/graph-api).
...
To initialize a static model from a model identifier string (for example, `openai:gpt-5`):
...
> 💡 Model identifier strings support automatic inference (for example, `"gpt-5"` will be inferred as `"openai:gpt-5"`).
> See the [reference](https://reference.langchain.com/python/langchain/models/#langchain.chat_models.init_chat_model(model)) for the full list of mappings.

For more control over configuration, initialize a model instance directly using the provider package.
In this example, we use [`ChatOpenAI`](https://reference.langchain.com/python/integrations/langchain_openai/ChatOpenAI).
...
Model instances give you complete control over configuration. Use them when you need to set specific [parameters](/oss/python/langchain/models#parameters) like `temperature`, `max_tokens`, `timeouts`, `base_url`, and other provider‑specific settings.
See the [provider reference](/oss/python/integrations/providers/all_providers) for available parameters and methods.
...
> ⚠️ **Pre‑bound models and structured output**
> Pre‑bound models (models with [`bind_tools`](https://reference.langchain.com/python/langchain_core/language_models/#langchain_core.language_models.chat_models.BaseChatModel.bind_tools) already called) are **not** supported when using structured output.
...
> For dynamic model selection patterns, see [Dynamic model in middleware](/oss/python/langchain/middleware#dynamic-model).
...
- **Reasoning**: Popularity is time‑sensitive, so the agent needs to use the search tool.
...
- **Reasoning**: The agent should confirm availability for the top‑ranked item.
...
- **Reasoning**: The agent has the most popular model and its stock status.
...
You can shape how your agent approaches tasks by providing a system prompt.
The [`system_prompt`](https://reference.langchain.com/python/langchain/agents/#langchain.agents.create_agent(system_prompt)) parameter can be provided as a string:
...
> 💡 For more details on message types and formatting, see [Messages](/oss/python/langchain/messages).
> For comprehensive middleware documentation, see [Middleware](/oss/python/langchain/middleware).

## Invocation

You can invoke an agent by passing an update to its [`State`](/oss/python/langgraph/graph-api#state).
All agents include a [sequence of messages](/oss/python/langgraph/use-graph-api#messagesstate) in their state; to invoke the agent, pass a new message:
...
This is more reliable but only works with providers that support native structured output (for example, OpenAI):
...
> 📝 As of `langchain 1.0`, simply passing a schema (for example, `response_format=ContactInfo`) is no longer supported.
> You must explicitly use `ToolStrategy` or `ProviderStrategy`.
...
Information stored in the state can be thought of as the agent’s [short‑term memory](/oss/python/langchain/short-term-memory).
...
Use middleware to define custom state when your custom state needs to be accessed by specific middleware hooks and tools attached to that middleware.
...
> Pydantic models and dataclasses are no longer supported. See the [v1 migration guide](/oss/python/migrate/langchain-v1#state-type-restrictions) for more details.

> 💡 To learn more about memory, see [Memory](/oss/python/concepts/memory).
> For information on implementing long‑term memory that persists across sessions, see [Long-term memory](/oss/python/langchain/long-term-memory).

### Streaming

We've seen how the agent can be called with `invoke` to get a final response.
If the agent executes multiple steps, this may take some time. To show intermediate progress, you can stream back messages as they occur:
...
- Process state before the model is called (for example, message trimming, context injection)
- Modify or validate the model's response (for example, guardrails, content filtering)
- Handle tool execution errors with custom logic
- Implement dynamic model selection based on state or context
- Add custom logging, monitoring, or analytics

Middleware integrates into the agent's execution graph, allowing you to intercept and modify data flow at key points without changing the core agent logic.
