...
```python 01_always_learn.py theme={null}
from agno.agent import Agent
...
Session 2 is a new conversation. Different session ID. But the agent remembers Alice.

**How it works:** When you set `learning=True`, the agent extracts user profile and memories in parallel with generating the response, stores them in the database, and recalls them in future sessions. No tool calls visible. The agent learns in the background.

## Agentic Learning

For more control, use agentic mode. The agent receives tools and decides what to save.

```python 02_agentic_learn.py theme={null}
from agno.agent import Agent
from agno.db.sqlite import SqliteDb
...
```python 03_learned_knowledge.py theme={null}
from agno.agent import Agent
from agno.db.sqlite import SqliteDb
...
<CardGroup cols={2}>
  <Card title="Your First Multi-Agent System" icon="code-branch" href="/first-multi-agent-system">
    Build your first multi-agent system.
  </Card>

  <Card title="Build Agents" icon="user-robot" href="/agents/overview">
    Configuration, tools, and advanced patterns
  </Card>
</CardGroup>
