...
from deepagents.backends import CompositeBackend, StateBackend, FilesystemBackend ## ⬅️ changed from StoreBackend to FilesystemBackend
...
# Checkpointer for state persistence within threads
# Note: For file storage, we use FilesystemBackend which saves directly to disk
...
def make_backend(runtime):
    ...
    - StateBackend: Ephemeral storage (lost when thread ends)
    - FilesystemBackend: Persistent storage on disk (survives across threads and restarts)
    """
    ## ⬇️ Create memories directory for persistent files
...
print(f"  - Checkpoints: {STORAGE_DIR / 'checkpoints.db'}")
print(f"  - Memories: {STORAGE_DIR / 'memories'}")
print("\nKey takeaways:")
...
print("  - All data is saved to disk and survives program restarts")
print("=" * 70)
